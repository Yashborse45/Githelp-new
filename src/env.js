...
// Central runtime + build-time env validation.
// Add any additional integration keys here as features expand.
const dev = process.env.NODE_ENV !== "production";
export const env = createEnv({
    server: {
        DATABASE_URL: z.string().url(),
        NODE_ENV: z.enum(["development", "test", "production"]).default("development"),
        // Clerk (allow missing in dev to unblock local work; runtime guard will throw if used)
        CLERK_SECRET_KEY: dev ? z.string().optional() : z.string(),
        CLERK_PUBLISHABLE_KEY: dev ? z.string().optional() : z.string(),
        // Pinecone (optional in dev if vector ops not run)
...
// Convenience export for NODE_ENV checks without importing zod types every time
